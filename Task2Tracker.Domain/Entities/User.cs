...
public class User : BaseEntity
{
    ...
    public User(string firstName, string lastName, string email, string passwordHash, UserRole role = UserRole.User)
    {
        ...
        if (string.IsNullOrWhiteSpace(passwordHash))
            throw new ArgumentException("Şifre hash boş olamaz.", nameof(passwordHash));
    ...
    public void UpdatePassword(string newPasswordHash)
    {
        if (string.IsNullOrWhiteSpace(newPasswordHash))
            throw new ArgumentException("Şifre hash boş olamaz.", nameof(newPasswordHash));
    ...
    private void ValidateAndSetName(string firstName, string lastName)
    {
        ...
        if (firstName.Length > MaxFirstNameLength)
            throw new ArgumentException($"İsim en fazla {MaxFirstNameLength} karakter olabilir.", nameof(firstName));

        if (lastName.Length > MaxLastNameLength)
            throw new ArgumentException($"Soyisim en fazla {MaxLastNameLength} karakter olabilir.", nameof(lastName));
    ...
    private void ValidateAndSetEmail(string email)
    {
        ...
        if (email.Length > MaxEmailLength)
            throw new ArgumentException($"E-posta en fazla {MaxEmailLength} karakter olabilir.", nameof(email));

        try { _ = new MailAddress(email.Trim()); }
        catch { throw new ArgumentException("Geçerli bir e-posta adresi giriniz.", nameof(email)); }
